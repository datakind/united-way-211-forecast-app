...
def merge_211_files(path: str, years: list, zip_file: str = None):
    ...
    for year in years:
        ...
        service_file = pd.read_excel(f'{path}/{year}/service_{year}.xls',
                                     sheet_name='Report Raw Data')
        service_file['count'] = service_file.groupby(
            'Transaction ID')['Transaction ID'].transform('count')
    ...
    data_frame_211['Contact Start Date'] = pd.to_datetime(
        data_frame_211['Contact Start Date'])
    ...
    if zip_file is not None:  # if zip code file provided, add zip code column
        ...
        try:
            ...
            logger.info('zip code file not found')
            raise

        zip_211 = pd.read_csv(f"{zip_file}")

        data_frame_211 = data_frame_211.merge(
            zip_211, how='left', left_on='Transaction ID', right_on='ContactID')

        data_frame_211 = data_frame_211.rename(
            columns={'SearchOptionZIPCode': 'zip'})
...
def map_211_service_category(data_frame_211: pd.DataFrame, map_file_path: str, verbose: bool = True):
    ...
    data_frame_211['Referred Services Category'] = data_frame_211['Referred Services'].map(
        service_dict)

    if verbose:
        map_percent = data_frame_211.loc[data_frame_211['Referred Services'].notna(
        ), 'Referred Services Category'].notna().mean()
        logger.info(
            f"{map_percent:.2%} of Referred Services map to high-level service category")
...
def create_211_volume(data_frame_211, zip_agg=True):
    ...
    data_frame_211['year'] = data_frame_211['Contact Start Date'].dt.year.astype(
        str)

    if zip_agg:

        total = data_frame_211.groupby(['zip', 'month', 'year']).size()
        house = data_frame_211.loc[data_frame_211['Referred Services Category'] == 'Housing'].groupby([
                                                                                                      'zip', 'month', 'year']).size()
        utx = data_frame_211.loc[data_frame_211['Referred Services Category']
                                 == 'Utility Assistance'].groupby(['zip', 'month', 'year']).size()
        food = data_frame_211.loc[data_frame_211['Referred Services Category']
                                  == 'Food/Meals'].groupby(['zip', 'month', 'year']).size()
        ...
        house = data_frame_211.loc[data_frame_211['Referred Services Category'] == 'Housing'].groupby([
                                                                                                      'month', 'year']).size()
        utx = data_frame_211.loc[data_frame_211['Referred Services Category']
                                 == 'Utility Assistance'].groupby(['month', 'year']).size()
        food = data_frame_211.loc[data_frame_211['Referred Services Category']
                                  == 'Food/Meals'].groupby(['month', 'year']).size()

    volume_211 = pd.concat([total, house, utx, food],
                           axis=1)

    volume_211.columns = ['total_calls',
                          'housing_calls', 'utx_calls', 'food_calls']
